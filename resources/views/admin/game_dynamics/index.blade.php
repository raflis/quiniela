    ...
    <div class="container-fluid">
        <div class="row layout-header">
            <div class="col-sm-12 header-content">
                <h1>
                    <i class="fas fa-bullhorn fa-xs text-white2"></i> Dinámicas
                </h1>
                <span class="subtitle">
            ...
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        ...
                        <a class="btn btn-success" href="{{ route('game_dynamics.create') }}">
                            <span class="icon">
                ...
                <div class="tablaTotal">
                    <table class="table tableD">
                        ...
                        <tbody>
                            @foreach ($game_dynamics as $item)
                            <tr>
                                <td>{{ $game_dynamics->firstItem() + $loop->index }}</td>
                                <td><img src="{{ $item->image }}" alt="" height=80></td>
                                <td>{{ $item->name }}</td>
                                <td>{{ $item->points }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->end_time)->format('d/m/Y H:i:s') }}</td>
                                <td>{{ ($item->draft == 0)?'Publicado':'Borrador' }}</td>
                                <td>{{ $item->order }}</td>
                                <td>
                                    <div style="display: inline-flex">
                                        <a class="btn btn-primary text-white btn-sm mr-1" href="{{ route('game_dynamics.edit', $item->id) }}">
                ...
                <div class="paginacionTotal d-flex justify-content-end">
                    {{ $game_dynamics->withQueryString()->render() }}
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-footer justify-content-center">
            ...
            {!! Form::open(['route' => ['game_dynamics.destroy', ''], 'method' => 'DELETE', 'id' => 'form-eliminar']) !!}
...
$('document').ready(function(){
    $('.btn-eliminar').click(function(e){
        ...
        var base = '{{ route('game_dynamics.destroy', '') }}';
        var url = base + '/' +id;
